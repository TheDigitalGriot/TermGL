...
//
// Ported from the reference JSX implementation (terminal-subpixel-demo-v2.jsx
// generatePrismScene). Uses an artistic 4-band color palette (blue, teal,
// green, amber), alpha-blended glass body with depth-dependent opacity,
// internal rainbow reflections, caustic shimmer, and a specular highlight
// on the top facet. The prism rotates continuously.
//
// Run with: go run ./examples/prism2
package main
...
// bandLerp interpolates through the 4-color palette. t in [0,1].
func bandLerp(t float64) (float64, float64, float64) {
	t = clampf(t, 0, 1)
	ct := t * 3.0 // 4 colors → 3 intervals
	i := int(ct)
	f := ct - float64(i)
	if i >= 3 {
		i = 3
...
// ============================================================================
// Per-frame rendering — direct port of generatePrismScene from JSX
// ============================================================================
...
func renderFrame(m *model) {
	...
	// Scale factor: the JSX was tuned for ~30 px scenes.
	// Multiply all absolute pixel constants by s so the visual
	// proportions are preserved at any terminal size.
	minDim := w
	if h < minDim {
		minDim = h
	}
	...
	// ── Prism geometry: equilateral triangle, continuously rotating ──
	prismAngle := t * 0.2
	prismSize := minDim * 0.28 // circumradius
	var tri [3]vec2
	for i := 0; i < 3; i++ {
	...
	for py := 0; py < m.vh; py++ {
		...
		for px := 0; px < m.vw; px++ {
			...
			if x > beamExitX-4*s {
				...
				totalSpread := spreadY * 2
				bandHeight := totalSpread / 4
			...
			// specular highlight on top facet (edge v[0] → v[1])
			if inside {
